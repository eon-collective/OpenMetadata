...
export interface EntityConversionResponse {
  entity: ConversionEntityReference;
  nodes?: ConversionEntityReference[];
  edges?: EdgeDetails[];
...
export type ConversionSourceType = Omit<SourceType, 'service'> & {
  direction: string;
  depth: number;
};
...
export interface ConversionEntityReference extends EntityReference {
  paging?: {
    entityDownstreamCount?: number;
    entityUpstreamCount?: number;
  };
...
export type ConversionNode = SearchSourceAlias & {
  nodeDepth?: number;
  paging: {
    entityDownstreamCount?: number;
    entityUpstreamCount?: number;
